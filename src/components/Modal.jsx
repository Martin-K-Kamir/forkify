...
const Modal = ({ children, className, renderClose, disableOverlay, ...rest }) => {
    ...
    const content = <div className={classes} data-fade-up={rest.isVisible}>
    ...
    return (
        <>
            {!disableOverlay && (
                <Overlay
                    {...rest}
                    renderClose={renderClose && isAboveMd}
                >
                    {content}
                </Overlay>
            )}
            {disableOverlay && content}
        </>
    );
};
